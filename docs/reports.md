...
| `producer_extension` | object or null | producer SHM extension collector | producer-defined, potentially sensitive |
...
The entire `crash_context` and `producer_extension` objects are typed values
...
`session_start_ns`, and an extension without values are represented by omitted
